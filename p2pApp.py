...
def netThrow(conn, secret, message,enc):
    ...
    try:
        if enc==0:
            ...
            conn.send(formatNumber(len(x_encode(message, secret,1))).encode())
            conn.send(x_encode(message, secret,1).encode())
...
def netCatch(conn, secret):
    ...
    try:
        data = conn.recv(4)
        if data.decode()[0] == '-':
            print(f"secret: {data.decode()[0]}")
            processFlag(data.decode(), conn)
            return 1
        data = conn.recv(int(data.decode()))
        stri=refract(xcrypt(data.decode(), bin(secret)[2:]))
        if stri[:5] =="SHARE":
            type1 = stri[5:]
            print("this is"+stri[5:])
            file_name = asksaveasfilename(
                title="Choose save location")
            try:
                f = open(file_name + type1, "wb")
                data = conn.recv(70000000)
                f.write(data)
                f.close()
            except IOError:
    ...
    if command == "nick":  # change nickname
        ...
        if isUsernameFree(param[0]):
            ...
            for conn in conn_array:
                ...
                netThrow(conn, secret_array[conn], param[0],1)
            username = param[0]
        else:
...
def placeText(text):
    ...
    for person in conn_array:
        netThrow(person, secret_array[person], text,1)
...
def File():
    filename= filedialog.askopenfilename(initialdir="/", title="Share File",
    filetypes=[ ('Any File', '*.*'),('Plain text', '*.txt')])
    global conn_array
    global secret_array
    global username
    for person in conn_array:
        for i in filename:
            if i==".":
                type2=filename[filename.index(i):]
                print(type2)
                break
        netThrow(person, secret_array[person], f"SHARE{type2}", 1)
    f = open(filename, 'rb')
    l = f.read(70000000)
    while (l):
        for person in conn_array:
            netThrow(person,secret_array[person],l,0)
        l = f.read(70000000)
    f.close()
...
if len(sys.argv) > 1 and sys.argv[1] == "-cli":
    ...
    statusConnect.set("Connect")
    filesharing = StringVar()
    filesharing.set("Choose file")
    clientType = 1
    ...
    fileshare = Button(root, text="Choose File",
                       command=File)
